...
app.get("/", (req, res) => {
  // Prova objetiva de revisão em execução no Cloud Run
  const service = process.env.K_SERVICE || "unknown_service";
  const revision = process.env.K_REVISION || "unknown_revision";

  res
    .type("text")
    .send(`clowdbot ok - v3-secure | service=${service} | revision=${revision}`);
});
...
app.get("/auth/linkedin", (req, res) => {
  try {
    ...
    return res
      .status(500)
      .type("text")
      .send(e instanceof Error ? e.message : "Unexpected error");
  }
});
...
app.get("/oauth/linkedin/callback", async (req, res) => {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      // Evite logar payload completo em produção.
      return res
    ...
    // checagens mínimas (MVP). Para produção, validar assinatura via JWKS.
